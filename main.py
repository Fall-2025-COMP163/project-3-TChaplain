...
def new_game():
    ...
    # ensure equipped slots and item_data exist for inventory logic
...
def load_game():
    ...
    # Reattach runtime-only fields expected by other modules
...
def game_loop():
    ...
    while game_running:
        ...
        # Save after each loop iteration to persist progress
        try:
            save_game()
        except Exception:
...
def view_character_stats():
    ...
    try:
        ...
        # If quest data is missing, just show lists
...
def quest_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            # Complete quest (useful for testing or linear flow)
...
def explore():
    ...
    # Build an enemy appropriate to player's level
...
def save_game():
    ...
    try:
        # Save using character_manager (it writes the expected fields)
        character_manager.save_character(current_character)
    except Exception as e:
        # Let the caller decide; here we print a warning
...
def load_game_data():
    ...
    try:
        ...
        # Re-raise so main can handle creating default files
        raise
        ...
        # Convert other unexpected exceptions to a format error
...
def handle_character_death():
    ...
    print("\n=== YOU DIED ===")
    # Offer revive: cost 50 gold (simple rule)
    revive_cost = 50
